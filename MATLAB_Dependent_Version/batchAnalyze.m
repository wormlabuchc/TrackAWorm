function batchAnalyze(StageData,fileData,savePath,overwriteOption,batchThreshold)
comp = computer;
...
batchThresholdData = cell([length(fileData) 1]);
...
for i=1:length(fileData)
    ...
    lastSlash = find(splineFile==slash);
    lastSlash = lastSlash(end);
    wormName = splineFile(lastSlash+1:end-11);
    imagePath = [splineFile(1:lastSlash) wormName];
...
%     MAXIMUM BEND
    ...
    if optionsMatrix(3)
        ...
        exc = {num2str(exc)};
        maxBendData(i) = exc;
        batchThresholdData{i}=batchThreshold;
        
    else
        
        maxBendData{i} = 'N/A';
        batchThresholdData{i} ='N/A';
...
labels=[{'Animal'} {'SplinePoint'} {'Bend'} {'Rec Length'} {'Avg Sum Bends'} {'Freq'} {'RMS'} {'Max Bend'} {'Max Bend Threshold'} {'Avg Amp'} {'A/L Ratio'} {'wormLength'} {'Avg Spd'} {'Speed F'} {'Speed B'} {'Tot Dist'} {'Net Dist'} {'Dist F'} {'Dist B'} {'Frames F'} {'Frames B'} {'Thrash Count'} {'Thrash Freq'} ];
newDataLine=[animalNameData splinePointData bendNumberData lengthOfRecordingData avgSumOfBendsData frequencyData rmsData maxBendData batchThresholdData amplitudeData avgSpeedData distanceData directionMetricsData thrashingData];
