...
function bendCursor_gui_OpeningFcn(hObject, eventdata, handles, varargin)
    ...
    axes(handles.win);
    cla reset;
    plot(time, bendAngles, 'LineWidth', 1); hold on;
    ...
    if ~isempty(filteredminTimes)
        plot(filteredminTimes, filteredNeg, 'ko', 'MarkerFaceColor','k', 'MarkerSize',8);
    end
   ...
   % Shift entire label (line + text) to the left
    xShift = 2.5;  % You can increase/decrease this to move entire label left
    
    % Position values
    x_base = max(time) - xShift;
    y_base = max(bendAngles) * 1.1;
    ...
    % Plot black label "Threshold" slightly to the right of the magenta line
    % Adjust horizontal shift based on font size (approx. 1 character ~ 6-7 units)
    text( ...
        x_base + 1, ...   % Fine-tuned horizontal offset (~6.5 units per dash × 6 dashes)
        y_base, ...
        'Threshold', ...
        'Color', 'k', ...
        'HorizontalAlignment', 'left', ...
