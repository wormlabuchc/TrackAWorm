classdef APana < handle
    ...
    methods
        function obj = APana  % it is a constructor funciton
            obj.gui = uifigure('Name','Action Potential app','Position',[10 50 1400 700], 'IntegerHandle', 'off', 'Resize', 'on','AutoResizeChildren','on');
            
            obj.gui.MenuBar = 'none';
            obj.gui.ToolBar = 'none';

            aplayout = uigridlayout(obj.gui);
            aplayout.RowHeight = {300,'1x','1x'};
            ...
            uicontrol(setpanel,'Style', 'text', 'Position', [90 420 90 30],'String','Cell Type','FontSize',15,'FontWeight','bold');

            obj.neuron_box = uicontrol(setpanel,'Style', 'checkbox', 'Position', [40 220 190 40],'String', '  Mammalian Neurons','FontSize',12,'FontWeight','bold','Callback',@obj.neuron_checkbox);
            uicontrol(setpanel,'Style', 'text', 'Position', [60 186 127 30],'String','Threshold detection','FontSize',10);
            uicontrol(setpanel,'Style', 'text', 'Position', [70 165 80 30],'String','Auto (dV/dt)','FontSize',10);

            uicontrol(setpanel,'Style', 'text', 'Position', [60 135 215 30],'String','Resting membrane potential search','FontSize',10);
            uicontrol(setpanel,'Style', 'text', 'Position', [60 115 130 30],'String','Time before peak ','FontSize',10);
            uicontrol(setpanel,'Style', 'text', 'Position', [265 115 20 30],'String','ms','FontSize',10);
            obj.mousebaseline = uicontrol(setpanel,'Style', 'edit', 'Position', [225 125 30 22],'String', 25 ...
                , 'FontSize', 10);
            ...
            obj.pasteBtn = uicontrol(setpanel,'Style', 'pushbutton', 'Position', [37 80 240 22],'FontName', 'arial','FontSize',10,...
        ...
        function runone(obj,~,~)  % for mouse Neuron runone analyze Ap
            ...
            if isempty(obj.line_ap)
                obj.line_ap = line(obj.ax_AP,'XData', x, 'YData', y, 'LineWidth', 1.5,'Color','k');
            else
                obj.line_ap.XData = x;
                obj.line_ap.YData = y;
            end
            ...
            obj.ax_AP.XLabel.String = 'Time (ms)';
            obj.ax_AP.YLabel.String = 'Membrane Potential (mV)';
            obj.ax_AP.YLim = [min(y)-10,max(y)+10];
            hold(obj.ax_trace,'on');
            %show peak point

            if isempty(obj.trace_AP_mark)
                obj.trace_AP_mark = drawpoint(obj.ax_trace, 'Position',[obj.locs(anaNo)/10000, obj.pks(anaNo)],'Color',[1 0 0],'StripeColor',"red");
            ...
            if isempty(obj.line_APD50)
                obj.line_APD50 = line(obj.ax_AP,'XData', [point_L.x, point_R.x], 'YData',[point_L.y, point_R.y], 'LineWidth', 1, 'Color','red');
            else
            ...
            if isempty(obj.apMerge_y)
                ...
                obj.ax_overlap.YLim = [min(obj.apMerge_y)-10, max(obj.apMerge_y)+10];
            else
                obj.apMerge_y = mean(cat(1,obj.apMerge_y, merge.y), 1);
                if isempty(obj.AP_merge)
                    obj.AP_merge = line(obj.ax_overlap,'XData',merge.x, 'YData', obj.apMerge_y, 'Color','blue','LineWidth',1.5);
                    obj.ax_overlap.YLim = [min(obj.apMerge_y)-10, max(obj.apMerge_y)+10];
                else
            ...
            if isempty(obj.pl_m_y)
                ...
                obj.ax_phase.YLim = [min(obj.pl_m_y)-10 max(obj.pl_m_y)+10];
                obj.ax_phase.XLim = [min(phaseplot_x)-10 max(phaseplot_x)+10];
            else
                obj.pl_m_y = cat(1,obj.pl_m_y, phaseplot_y);
                obj.pl_m_x = cat(1,obj.pl_m_x, phaseplot_x);
                obj.phase_merge.YData = mean(cat(1,obj.pl_m_y, phaseplot_y),1);
                obj.phase_merge.XData = mean(cat(1,obj.pl_m_x, phaseplot_x),1);
                obj.ax_phase.YLim = [min(obj.phase_merge.YData)-10 max(obj.phase_merge.YData)+10]
                obj.ax_phase.XLim = [min(obj.phase_merge.XData)-10 max(obj.phase_merge.XData)+10]
            end
        ...
        function runonebwmforward(obj, ~, ~)  % for C.elegans BWM runone analyze A
            ...
            if 0==obj.AP_count.Value & obj.locs(1)>100
                anaNo = 1;
                ppmsval=0;
                cla(obj.ax_overlap);
                obj.AP_merge = []; 
            ...
            x = 0.1:0.1:(length(y)/10);
            if isempty(obj.line_ap)
                obj.line_ap = line(obj.ax_AP,'XData', x, 'YData', y, 'LineWidth', 1,'Color','k');
            else
                obj.line_ap.XData = x;
                obj.line_ap.YData = y;
            end
            hold(obj.ax_trace,'on');
            if isempty(obj.trace_AP_mark)
                obj.trace_AP_mark = drawpoint(obj.ax_trace, Position=[obj.locs(anaNo)/1000, obj.pks(anaNo)],Color='red');
            else
            ...
            if isempty(obj.apMerge_y)
                ...
                obj.apMerge_y = mean(cat(1,obj.apMerge_y, merge.y), 1);
                if isempty(obj.AP_merge)
                    obj.AP_merge = line(obj.ax_overlap,'XData',merge.x, 'YData', obj.apMerge_y, 'Color','blue','LineWidth',1);
                else
                    obj.AP_merge.YData = obj.apMerge_y;
                end
                uistack(obj.AP_merge,'top');
           ...
           phaseplot_x = phase_y(1:end-1);
            ...
            if isempty(obj.pl_m_y)
                obj.pl_m_y = phaseplot_y;
                obj.pl_m_x = phaseplot_x;
                obj.phase_merge = line(obj.ax_phase,'XData', phaseplot_x, 'YData', smooth(obj.pl_m_y,3), 'Color', 'blue', 'LineWidth', 1);
                obj.ax_phase.YLim = [min(phaseplot_y)-2, max(phaseplot_y)+2];
                obj.ax_phase.XLim = [min(phaseplot_x)-5, max(phaseplot_x)+5];
            else
                ...
                obj.phase_merge.YData = mean(cat(1,obj.pl_m_y, phaseplot_y),1);
                obj.phase_merge.XData = mean(cat(1,obj.pl_m_x, phaseplot_x),1);
                obj.ax_phase.YLim = [min(obj.phase_merge.YData)-5, max(obj.phase_merge.YData)+5];
                obj.ax_phase.XLim = [min(obj.phase_merge.XData)-5, max(obj.phase_merge.XData)+5];
            end
            uistack(obj.phase_merge,'top');
        ...
        function pasteData(obj,~,~)
            ...
            plot(obj.ax_trace, obj.ap_x, obj.ap_y,'k');
            obj.ax_trace.YLim =[min_y,max_y];
            obj.ax_trace.XLim =[0,ceil(data_length/10000)];
            ...
            [obj.pks,obj.locs] = findpeaks(obj.ap_y, 'MinPeakHeight',0);
         ...
         function pasteDataBWM(obj,~,~)
            ...
            obj.ax_trace.Box = 'off';
            [obj.pks,locs] = findpeaks(obj.ap_y, 'MinPeakHeight',0,'MinPeakDistance',800);
            hold(obj.ax_trace,"on");
            obj.locs = locs/10;
        ...
        function neuron_checkbox(obj,~,~)
                ...
                obj.ax_phase.YTick = -60:30:120;
                obj.ax_phase.YLim = [-60 120];
        ...
        function bwm_checkbox(obj,~,~)
            toreset(obj);
                ...
                obj.runoneAP.ButtonPushedFcn = @obj.runonebwmforward;
                obj.runonebackbtn.ButtonPushedFcn = @obj.runonebwmback;
                obj.bwmrmpline = [];


                obj.anaBtn.ButtonPushedFcn = @obj.runAnaBWM;
        ...
        function exportdata(obj,~,~)
            % phasedata = cat(1,obj.pl_m_x,obj.pl_m_y).';
             phasedata = cat(1,obj.phase_merge.XData,obj.phase_merge.YData).';
            ...
            ActiveSheet.Range('B1:S1').Value={['AP#'],['RMP (mV)'], ['Threshold (mV)'], ['Amp (mV)'], ['APD50 (ms)'],['AHP (mV)'],['Rise time (ms)'],['Decay time (ms)'],['Rise slope (mV/ms)'],['Decay slope (mV/ms)'], ['RiseSlope(10-90%)(mV/ms)'], ['DecaySlope(10-90%)(mV/ms)'],[''],['Membrane Potential (mV)'],['Slope (mV/ms)'],[''],['Time (ms)'], ['Membrane Potential (mV)']};
