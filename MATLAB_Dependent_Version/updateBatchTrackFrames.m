...
% msg = sprintf('Resolution of first image: %d x %d pixels', width, height);
% disp(msg);
...
for i=1:10
    ...
    if currentFrame~=0 && exist('x','var') && ~isempty(x)
        ...
        if ~isempty(ventralDir) && ventralDir(currentFrame)
            ...
            %quiver(windowHandles(i),startPt(1),res(1)-startPt(2)+1,arrowComponents(1),arrowComponents(2),'MaxHeadSize',30,'Color','r','LineWidth',1);
