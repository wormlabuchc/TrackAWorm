...
set(handles.totalFrames,'Visible','off')
...
switch savePath
    ...
    if ~isnumeric(splinePath)
        ...
        imagePath = [splinePath(1:lastSlash) wormName];
        firstImage = getFirstNameImage(imagePath);
        imagePath = [imagePath slash firstImage];

        %Get image information
        imageInfo = imfinfo(imagePath);
        width = imageInfo.Width;
        height = imageInfo.Height;
        res_analysis = [width height];
        ...
        set(handles.recordCalib,'String',num2str(calib));
        %set(handles.calib,'String',num2str(calib));
        if res_analysis(1) == 728 
            set(handles.calib,'String',num2str(2*calib));
        else
            set(handles.calib,'String',num2str(calib));
        end
...
%set(hObject,'BackgroundColor',get(gcf,'Color'));
...
[splineFile,~,~,framerate,~,~,framesToAnalyze,~] = getAnalysisProperties(handles);
bendThreshold = str2double(get(handles.bendThreshold, 'String'));
...
exc = bendCursor_gui(time,selectedBendAngles,bendThreshold);
...
elseif numBins < 0 
    msgbox('Number of bins can''t be negative values ', 'No Bins', 'warn');
...
labels=[{'Animal'} {'splinePoint'} {'Bend'} {'Rec Length (s)'} {'Avg Sum Bends (deg)'} {'Freq (Hz)'} {'RMS (deg)'} {'Max Bend (deg)'} {'Max Bend Threshold'} {'Avg Amp (µm)'} {'A/L Ratio'} {'wormLength (µm)'} {'Avg Spd (µm/s)'} {'Speed F (µm/s)'} {'Speed B (µm/s)'} {'Tot Dist (µm)'} {'Net Dist (µm)'} {'Dist F (µm)'} {'Dist B (µm)'} {'Frames F'} {'Frames B'} {'Thrash Count'} {'Thrash Freq (/min)'} {'Comments'}];
newDataLine=[animalName {splinePoint} {num2str(bendNumber)} lengthOfRecording sumOfAngles freq rms exc bendThreshold meanAmp alratio length meanSpeed speedForward speedBackward totalDistance netDistance distForward distBackward stepsForward stepsBackward thrashCount thrashFreq comments];
% size(labels)
% size(newDataLine)
...
set(handles.movementMessages,'String','Forward/Backward Speed Plot data exported')
...
function bendThreshold_Callback(hObject, eventdata, handles)
% hObject    handle to bendThreshold (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject,'String') returns contents of bendThreshold as text
%        str2double(get(hObject,'String')) returns contents of bendThreshold as a double
flashBox('g',handles.bendThreshold);
set(handles.maxBend,'Enable','on');

% --- Executes during object creation, after setting all properties.
function bendThreshold_CreateFcn(hObject, eventdata, handles)
% hObject    handle to bendThreshold (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
function recordCalib_Callback(hObject, eventdata, handles)
% hObject    handle to recordCalib (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject,'String') returns contents of recordCalib as text
%        str2double(get(hObject,'String')) returns contents of recordCalib as a double
% IF NEW CALIB VALUE IF APPROPRIATE, THEN SAVE IT -- ELSE RETURN TO
% EXISTING VALUE
...
% --- Executes during object creation, after setting all properties.
function recordCalib_CreateFcn(hObject, eventdata, handles)
% hObject    handle to recordCalib (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
% --- Executes during object creation, after setting all properties.
function selectedFrames_CreateFcn(hObject, eventdata, handles)
% hObject    handle to selectedFrames (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
