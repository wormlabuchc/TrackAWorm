function locomotionData = newWormLocomotion(splineFile,stageFile,timesFile,framerate,reducedframerate,calib,point,framesToAnalyze,res,corerate)
...
if framerate==5&&reducedframerate==3
    framerate=3;
elseif framerate==5&&reducedframerate==1
    framerate=1;
elseif framerate==3&&reducedframerate==1
    framerate=1;
end
% 
...
proj_New={proj_New};
forwardSpeed_New=forwardSpeed;
% Multiply array2 by -1
backwardSpeed_New = -1 * backwardSpeed;
% Replace NaN values in array1 with corresponding values from array2
forwardSpeed_New(isnan(forwardSpeed_New)) = backwardSpeed_New(isnan(forwardSpeed_New));
forwardSpeed_New={forwardSpeed_New};
times={times};

% figure; 
% plot(times,forwardSpeed)
% hold on
% scatter(times,forwardSpeed,20,"filled","MarkerFaceColor","#0072BD")
% hold off
% xlabel('time (sec)');
% ylabel('Speed (um/sec)');
% % Add a title
% title('Forward vs Backword Speeds of AVA mutants');
...
locomotionData = struct('Amplitude',amplitudeData,'Speed',speedData,'Direction',directionData,'Count',countData,'Plot',plotData,'Distance',distanceData,'times',times,'forwardSpeed',forwardSpeed_New,'proj',proj_New);